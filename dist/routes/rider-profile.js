...
exports.riderProfileRouter = (0, express_1.Router)();
const AVATAR_BUCKET = 'avatars';
// Columns a rider may read about themselves (never password / verification_code).
const PUBLIC_COLUMNS = 'id, full_name, email, phone_number, is_verified, status, created_at, vehicle_type, vehicle_brand, vehicle_model, vehicle_year, vehicle_plate, vehicle_battery_capacity, payout_bank, payout_account_number, payout_bvn, license_path, license_front_path, license_back_path, selfie_path, selfie_with_license_path, avatar_url';
...
exports.riderProfileRouter.post('/', rider_auth_1.requireRider, (0, async_handler_1.asyncHandler)(async (req, res) => {
    const body = (req.body ?? {});
    const riderId = req.riderId;
    const action = typeof body.action === 'string' ? body.action : 'get';
    if (action === 'get') {
        const { data, error } = await supabase_1.admin
            .from('riders')
    ...
    if (action === 'sign_avatar_upload') {
        const ext = typeof body.ext === 'string' && /^[a-z0-9]{1,5}$/i.test(body.ext)
        ...
        if (error)
            return res.status(400).json({ error: error.message });
        const { data: pub } = supabase_1.admin.storage.from(AVATAR_BUCKET).getPublicUrl(path);
