...
exports.geoRouter.post('/', user_auth_1.requireUser, (0, async_handler_1.asyncHandler)(async (req, res) => {
    if (!env_1.env.googleMapsApiKey) {
    ...
    try {
        if (action === 'autocomplete') {
            ...
            if (typeof body.session === 'string')
                params.sessiontoken = body.session;
            if (typeof body.country === 'string')
                params.components = `country:${body.country}`;
            const data = await (0, google_1.gmaps)('place/autocomplete/json', params);
        ...
        if (action === 'details') {
            ...
            if (typeof body.session === 'string')
                params.sessiontoken = body.session;
            const data = await (0, google_1.gmaps)('place/details/json', params);
            if (data.status !== 'OK') {
        ...
        if (action === 'directions') {
            ...
            const data = await (0, google_1.gmaps)('directions/json', {
                origin: `${o.lat},${o.lng}`,
            ...
            if (!leg)
                return res.status(400).json({ error: 'no_route' });
            return res.status(200).json({
                ok: true,
                route: {
