...
export const riderDocumentsRouter = Router();
...
riderDocumentsRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!column) return res.status(400).json({ error: 'invalid_doc_type' });

    // Path is always scoped under the rider's own id folder.
    const ext =
      typeof body.ext === 'string' && /^[a-z0-9]{1,5}$/i.test(body.ext) ? body.ext : 'jpg';
